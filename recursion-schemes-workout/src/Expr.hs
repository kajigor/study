-- The source: http://comonad.com/reader/2009/incremental-folds/
...
import Data.Semigroup.Reducer hiding (cons)
...
newtype Size = Size { getSize :: Int } deriving (Eq,Ord,Show,Read)
  deriving Num
    via Int
newtype Sum = Sum { getSum :: Int } deriving (Eq,Ord,Show,Read)
  deriving Num
    via Int
...
newtype Mon m = Mon { getMon :: m } deriving (Eq,Ord,Show,Read)

instance Semigroup m => Semigroup (Mon m) where
  (Mon x) <> (Mon y) = Mon $ x <> y

instance Monoid m => Monoid (Mon m) where
  mempty = mempty

instance (a `Reducer` m, Monoid m) => Algebra (Tree a) (Mon m) where
...
instance (a `Reducer` m, Monoid m) => Algebra (List a) (Mon m) where
